...
class RbacMiddleware(MiddlewareMixin):
    def process_request(self,request):
        ...
        for reg in settings.RBAC_NO_PERMISSION_LIST:
            if re.match(reg,request.path_info):
                setattr(request, settings.RBAC_RBCORD_LIST, record_list)
                return None
        ...
        for name,url_info in permission_dict.items():
            ...
            if re.match(url,request.path_info):
                ...
                if parent:
                    setattr(request,settings.RBAC_CURRENT_PARENT_KEY,parent)
                    record_list.append(permission_dict[parent])
                    record_list.append(url_info)
                else:
                    setattr(request, settings.RBAC_CURRENT_PARENT_KEY, name)
                    record_list.append(url_info)
                setattr(request,settings.RBAC_RBCORD_LIST,record_list)
                return None
